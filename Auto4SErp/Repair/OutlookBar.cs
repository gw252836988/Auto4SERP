...
namespace OutLookBarDemo
{
    public class OutlookBar : Panel
    {
        ...
        public void AddBand(string caption, ContentPanel content, Image img)
        {
            content.outlookBar = this;
            int index = Controls.Count;
            BandTagInfo bti = new BandTagInfo(this, index);
            BandPanel bandPanel = new BandPanel(caption, content, bti, img);
        ...
        private void SizeChangedEvent(object sender, EventArgs e)
        {
            ...
            Size = new Size(180, ((Control)sender).ClientRectangle.Size.Height);
            UpdateBarInfo();
            RedrawBands();
        }
    }
    ...
    {
        public BandPanel(string caption, ContentPanel content, BandTagInfo bti, Image img)
        {

            BandButton bandButton = new BandButton(caption, bti, img);
            bandButton.Visible = false;
            Controls.Add(bandButton);
            Controls.Add(content);
        }
    }

    internal class BandButton : DevComponents.DotNetBar.ButtonX
    {
        private BandTagInfo bti;

        public BandButton(string caption, BandTagInfo bti, Image img)
        {
            Text = caption;
            //FlatStyle=FlatStyle.Standard;
            Style = DevComponents.DotNetBar.eDotNetBarStyle.Office2003;
            Visible = true;
            this.bti = bti;
    ...
    public class IconPanel : ContentPanel
    {
        ...
        public IconPanel()
        {
            margin = 20;
            //这里是调节图标间距的
            iconSpacing = 48 + 15 + 20;	// icon height + text height + margin
            BackColor = Color.Transparent;
            AutoScroll = true;
        }

        public void AddIcon(string caption, Image image, EventHandler onClickEvent, string funN)
        {
            int index = Controls.Count / 2;	// two entries per icon
            PanelIcon panelIcon = new PanelIcon(this, image, index, onClickEvent, funN);
    ...
    public class PanelIcon : PictureBox
    {
        ...
        public PanelIcon(IconPanel parent, Image image, int index, EventHandler onClickEvent, string funN)
        {
            this.index = index;
